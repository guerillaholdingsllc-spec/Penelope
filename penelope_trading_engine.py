...
# ── v3.9 risk + performance constants ────────────────────────────
DAILY_LOSS_LIMIT_PCT  = 0.03   # halt all trading if portfolio drops 3% in one day
TAKE_PROFIT_ATR_MULT  = 2.5    # exit long when price rises 2.5×ATR above entry
TRAILING_STOP_ATR     = 1.2    # trailing stop distance = 1.2×ATR (tighter than entry stop)
MAX_HOLD_BARS         = 48     # force-exit any position held longer than 48 hourly bars
MIN_PROFIT_FACTOR     = 1.0    # block new entries if strategy profit factor < 1.0
...
def signal_rsi(symbol: str, period=14) -> str:
    ...
    # Adaptive RSI thresholds — shift with market regime (v4.0)
    _regime = detect_market_regime(bars) if len(bars) >= 20 else "neutral"
    if _regime == "bull":
        _buy_thresh = 40    # relaxed — trend is up, buy dips earlier
    elif _regime == "bear":
        _buy_thresh = 25    # strict — only buy extreme oversold in downtrend
    else:
        _buy_thresh = 32    # original threshold in choppy/neutral market
    if rsi < _buy_thresh: return "buy"   # adaptive (v4.0)
...
def execute_strategy(s: dict, st: dict, state: dict = None):
    ...
    # ── Exit logic: ATR stop / trailing stop / take-profit / time exit (v3.9) ──
    if pos:
        ...
        # 1. Take-profit: price rose TAKE_PROFIT_ATR_MULT × ATR above entry
        if atr_val > 0:
            take_profit_level = entry_price + TAKE_PROFIT_ATR_MULT * atr_val
            if price >= take_profit_level:
                exit_reason = f"TAKE PROFIT @ ${price:.2f} (target=${take_profit_level:.2f})"

        # 2. Trailing stop: price fell TRAILING_STOP_ATR × ATR below peak
        if not exit_reason and atr_val > 0:
            trail_stop = peak - TRAILING_STOP_ATR * atr_val
            if price <= trail_stop:
                exit_reason = f"TRAILING STOP @ ${price:.2f} (trail=${trail_stop:.2f} peak=${peak:.2f})"

        # 3. Hard ATR stop: price fell ATR_STOP_MULT × ATR below entry (original)
        if not exit_reason and atr_val > 0:
            hard_stop = entry_price - ATR_STOP_MULT * atr_val
            if price <= hard_stop:
                exit_reason = f"ATR STOP HIT @ ${price:.2f} (stop=${hard_stop:.2f})"

        # 4. Time-based exit: held too long
        if not exit_reason and bars_held >= MAX_HOLD_BARS:
            exit_reason = f"TIME EXIT @ ${price:.2f} (held {bars_held}/{MAX_HOLD_BARS} bars)"
        ...
        if exit_reason:
            ...
            if result.get("id"):
                ...
                won        = trade_pnl > 0
                # Slippage tracking
                slip = track_slippage(exit_price, exit_price, "sell", qty)
                st.setdefault("trade_pnls", []).append(round(pnl_pct, 5))
                if len(st["trade_pnls"]) > 50:
                    st["trade_pnls"] = st["trade_pnls"][-50:]
                # Update win/loss counts
                if won:
                    st["win_count"] = st.get("win_count", 0) + 1
                else:
                    st["consecutive_losses"] = st.get("consecutive_losses", 0) + 1
                    check_consecutive_losses(sid, st)
                # Update peak equity watermark
                new_cash = round(cash + qty * exit_price, 2)
                st["peak_equity"] = max(st.get("peak_equity", new_cash), new_cash)
                st["position"]    = None
                st["cash"]        = new_cash
                st["trade_count"] = st.get("trade_count", 0) + 1
                # Compute performance metrics
                pf  = calc_profit_factor(st["trade_pnls"])
                exp = calc_expectancy(st["trade_pnls"], st.get("initial_capital", 20000))
                wlr = calc_win_loss_ratio(st["trade_pnls"])
                log_fn = log.info if won else log.warning
                log_fn(f"[{sid}] EXIT: {exit_reason} | PnL=${trade_pnl:+.2f} "
                       f"({'WIN' if won else 'LOSS'}) | "
                       f"PF={pf:.2f} Exp=${exp:+.2f} WLR={wlr:.2f} "
                       f"slip={slip['slip_bps']:.1f}bps")
                # Persist to SQLite ledger (survives restarts) ─── v4.0
                log_trade(sid, symbol, "long", qty, entry_price, exit_price,
                          trade_pnl, pnl_pct, exit_reason)
                ledger_pnls = get_ledger_pnls(sid)
                if len(ledger_pnls) >= 3:
                    st["trade_pnls"] = ledger_pnls[-50:]  # reload from DB
                post_feed(f"EXIT {symbol}", f"{exit_reason} | PnL=${trade_pnl:+.2f}", 
                          "success" if won else "warning")
                telegram(f"{'📈' if won else '📉'} *EXIT* `{symbol}` | {exit_reason} | "
                         f"PnL=${trade_pnl:+.2f} | PF={pf:.2f}")
            return
    ...
    # ── Performance metrics snapshot (v3.9) ────────────────────
    pnls_snap = st.get("trade_pnls", [])
    pf_snap   = calc_profit_factor(pnls_snap) if len(pnls_snap) >= 3 else 0.0
    exp_snap  = calc_expectancy(pnls_snap, capital) if len(pnls_snap) >= 3 else 0.0
    wlr_snap  = calc_win_loss_ratio(pnls_snap) if len(pnls_snap) >= 3 else 0.0
    peak_eq   = st.get("peak_equity", current_value)
    cur_dd    = round((peak_eq - current_value) / peak_eq * 100, 2) if peak_eq > 0 else 0.0
    log.info(f"[{sid}] signal={signal} value=${current_value:.2f} pnl={st['pnl_pct']:+.1f}% | "
             f"PF={pf_snap:.2f} Exp=${exp_snap:+.2f} WLR={wlr_snap:.2f} DD={cur_dd:.1f}%")

    if signal == "buy" and not pos and cash >= MIN_BUY_DOLLARS:
        # ── Daily loss limit gate (v3.9) ────────────────────────
        portfolio_equity = sum(
            v.get("portfolio_value", v.get("initial_capital", 0))
            for k,v in (state or {}).items()
            if k != "_meta" and isinstance(v, dict)
        )
        if check_daily_loss_limit(portfolio_equity):
            log.info(f"[{sid}] DAILY LOSS LIMIT — trading halted for today")
            return

        # ── Profit factor gate (v3.9) ───────────────────────────
        # ── Multi-timeframe confirmation (v4.0) ────────────────────────────
        _ah = {"APCA-API-KEY-ID":     os.environ.get("ALPACA_API_KEY",""),
               "APCA-API-SECRET-KEY": os.environ.get("ALPACA_SECRET_KEY","")}
        _mtf = get_4h_trend(symbol, _ah)
        if _mtf == "down":
            log.info(f"[{sid}] MTF BLOCK: 4H trend=down vs 1H buy signal — skip")
            return

        pf_blocked, pf_reason = profit_factor_gate(st)
        log.info(f"[{sid}] {pf_reason}")
        if pf_blocked:
            return
...
# ── Performance metrics (v3.9) ────────────────────────────────────

def calc_profit_factor(trade_pnls: list) -> float:
    """Gross profit / gross loss. >1.5 = good, <1.0 = losing."""
    if not trade_pnls:
        return 0.0
    gross_win  = sum(p for p in trade_pnls if p > 0)
    gross_loss = abs(sum(p for p in trade_pnls if p < 0))
    return round(gross_win / gross_loss, 3) if gross_loss > 0 else (999.0 if gross_win > 0 else 0.0)


def calc_expectancy(trade_pnls: list, capital: float = 1.0) -> float:
    """
    ...
    lr       = 1 - wr
    return round((avg_win * wr - avg_loss * lr) * capital, 2)


def calc_win_loss_ratio(trade_pnls: list) -> float:
    """avg_win / avg_loss. >1.5 preferred."""
    wins   = [p for p in trade_pnls if p > 0]
...
def monte_carlo_simulation(trade_pnls: list, n_sims: int = 1000,
                           capital: float = 20000) -> dict:
    """
    Run N Monte Carlo simulations by randomly shuffling trade sequence.
    Returns: worst_dd%, median_final, pct_profitable_runs
    """
    import random
    if len(trade_pnls) < 5:
        return {"worst_dd": 0.0, "median_final": capital, "pct_profitable": 0.0}
    results = []
    for _ in range(n_sims):
        shuffled = random.sample(trade_pnls, len(trade_pnls))
        cap = capital; peak = capital; max_dd = 0.0
        for pnl_pct in shuffled:
            cap *= (1 + pnl_pct)
            peak = max(peak, cap)
            dd = (peak - cap) / peak * 100
            max_dd = max(max_dd, dd)
        results.append({"final": cap, "max_dd": max_dd})
    results.sort(key=lambda x: x["final"])
    profitable = sum(1 for r in results if r["final"] > capital)
    return {
        "worst_dd":       round(max(r["max_dd"] for r in results), 2),
        "median_final":   round(results[n_sims//2]["final"], 2),
        "pct_profitable": round(profitable / n_sims * 100, 1),
        "worst_final":    round(results[0]["final"], 2),
    }


# ── Daily loss limit state (v3.9) ─────────────────────────────────
_daily_loss_state = {
    "date":          None,
    "start_equity":  None,
    "halted":        False,
...
def check_daily_loss_limit(current_equity: float) -> bool:
    """
    Returns True (halted) if portfolio dropped > DAILY_LOSS_LIMIT_PCT today.
    Resets at midnight UTC.
    """
    today = datetime.now(timezone.utc).date().isoformat()
    if _daily_loss_state["date"] != today:
        _daily_loss_state.update({"date": today,
                                   "start_equity": current_equity,
                                   "halted": False})
        return False
    if _daily_loss_state["start_equity"] and _daily_loss_state["start_equity"] > 0:
        loss_pct = ((_daily_loss_state["start_equity"] - current_equity)
                    / _daily_loss_state["start_equity"])
        if loss_pct >= DAILY_LOSS_LIMIT_PCT:
            if not _daily_loss_state["halted"]:
                log.warning(f"DAILY LOSS LIMIT HIT: "
                            f"loss={loss_pct*100:.2f}% >= {DAILY_LOSS_LIMIT_PCT*100:.0f}% — "
...
def profit_factor_gate(st: dict) -> tuple:
    """
    Block new entry if strategy profit factor < MIN_PROFIT_FACTOR.
    Returns (blocked: bool, reason: str)
    """
    pnls = st.get("trade_pnls", [])
    if len(pnls) < 5:
        return False, "pf_gate: insufficient history (<5 trades)"
    pf = calc_profit_factor(pnls)
    if pf < MIN_PROFIT_FACTOR:
        return True, f"PF GATE BLOCK: profit_factor={pf:.3f} < {MIN_PROFIT_FACTOR}"
    return False, f"pf_gate_ok: PF={pf:.3f}"


def track_slippage(expected_price: float, filled_price: float,
                   side: str, qty: float) -> dict:
    """
    Track execution slippage vs expected price.
    Returns slippage in dollars and basis points.
    """
    if side == "buy":
        slip_per_share = filled_price - expected_price   # positive = paid more
    else:
        slip_per_share = expected_price - filled_price   # positive = received less
    slip_dollars = round(slip_per_share * qty, 4)
    slip_bps     = round(slip_per_share / expected_price * 10000, 2) if expected_price else 0
    return {"slip_dollars": slip_dollars, "slip_bps": slip_bps, "slip_per_share": round(slip_per_share, 4)}


import sqlite3 as _sqlite3

LEDGER_PATH = Path("/root/workspace/Penelope/trading_bot/trade_ledger.db")

def init_ledger():
    """Create trade ledger DB if not exists."""
...
def log_trade(strategy: str, symbol: str, side: str, qty: float,
              entry: float, exit_price: float, pnl_usd: float,
              pnl_pct: float, exit_reason: str = ""):
    """Persist a closed trade to the ledger."""
    try:
        init_ledger()
        conn = _sqlite3.connect(str(LEDGER_PATH))
        conn.execute(
            "INSERT INTO trades (ts,strategy,symbol,side,qty,entry,exit,pnl_usd,pnl_pct,exit_reason) "
            "VALUES (?,?,?,?,?,?,?,?,?,?)",
            (datetime.now(timezone.utc).isoformat(), strategy, symbol, side,
             qty, entry, exit_price, round(pnl_usd,2), round(pnl_pct,5), exit_reason)
        )
        conn.commit()
        conn.close()
    except Exception as e:
...
def get_ledger_pnls(strategy: str) -> list:
    """Load historical pnl_pct list from ledger (survives restarts)."""
    try:
        if not LEDGER_PATH.exists():
            return []
        conn = _sqlite3.connect(str(LEDGER_PATH))
        rows = conn.execute(
            "SELECT pnl_pct FROM trades WHERE strategy=? ORDER BY id",
            (strategy,)).fetchall()
        conn.close()
...
def get_4h_trend(symbol: str, headers: dict) -> str:
    """
    Fetch 4H bars and determine trend direction.
    ...
    try:
        ...
        r = _req.get(
            ...
            params={"timeframe":"4Hour","limit":20,"adjustment":"raw",
                    "start":"2026-03-01T13:30:00Z"},
            headers=headers, timeout=10)
        if r.status_code != 200:
            return "neutral"
        bars_4h = r.json().get("bars", [])
        if len(bars_4h) < 5:
            return "neutral"
        # Simple: price vs 10-bar SMA on 4H
        closes  = [b["c"] for b in bars_4h[-10:]]
...
def walk_forward_report(state: dict, cycle: int):
    ...
    # Run Monte Carlo on top strategy
    if rankings:
        top = rankings[0]
        top_pnls = state.get(top["sid"], {}).get("trade_pnls", [])
        if len(top_pnls) >= 5:
            mc = monte_carlo_simulation(top_pnls, n_sims=500,
                                         capital=state.get(top["sid"], {}).get("initial_capital", 20000))
            log.info(f"MONTE CARLO [{top['sid']}]: "
                     f"worst_dd={mc['worst_dd']:.1f}% "
                     f"median_final=${mc['median_final']:,.0f} "
                     f"profitable_runs={mc['pct_profitable']:.0f}%")
...
def main():
    ...
    post_feed("Engine v4.0 Online", f"Trading engine v4.0 started (MTF+AdaptiveRSI+Ledger) (TrailingStop+TakeProfit+MonteCarl+DailyLimit) (MinBuy$3k-$5k+QualityGate) (OptimizedThresholds) (PortfolioCap+VolAdj+Benchmark). {len(STRATEGIES)} strategies.", "success")
