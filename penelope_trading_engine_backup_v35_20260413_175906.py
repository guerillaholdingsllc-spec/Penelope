...
def execute_strategy(s: dict, st: dict, state: dict = None):
    ...
    if signal == "buy" and not pos and cash >= 200:
        ...
        qty = round(kelly_qty * t_mult, 4)             # kelly × temporal
        if qty < 0.001:
            return
...
def walk_forward_report(state: dict, cycle: int):
    ...
    for sid, st in state.items():
        ...
        sharpe = calc_sharpe(pnls) if pnls else 0.0
    ...
    for rank, r in enumerate(rankings, 1):
        ...
        log.info(f"  #{rank} {r['sid']}: Sharpe={r['sharpe']:+.2f} "
...
def main():
    ...
    post_feed("Engine v3.5 Online", f"Trading engine v3.5 started (Kelly+WalkForward). {len(STRATEGIES)} strategies.", "success")
