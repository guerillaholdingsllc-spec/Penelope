...
def telegram(msg: str, force=False):
    """All non-critical messages dropped. Use send_critical for real emergencies."""
    from telegram_gate import notify as _g
    _g(msg)
...
def signal_momentum(symbol: str) -> str:
    ...
    log.info(f"[momentum/{symbol}] ROC={roc:.2f}%")
    if roc > 2.0:  return "buy"
    if roc < -2.0: return "sell"
    return "hold"
...
def main():
    ...
    post_feed("Engine v3.1 Online", f"Trading engine v3.1 started. {len(STRATEGIES)} strategies.", "success")
