    ...
    if signal == "buy" and not pos and cash >= 200:
        ...
        if state is not None:
            ...
            if port_blocked:
                ...
                return
        # ── Kelly × temporal × volatility-adjusted size ───────────────
        kelly_qty = kelly_position_size(st, cash, price)
        vol_mult  = vol_size_multiplier()              # shrink in high-vol
        qty = round(kelly_qty * t_mult * vol_mult, 4)  # kelly×temporal×vol
        log.info(f"[{sid}] Size: kelly_qty={kelly_qty} t_mult={t_mult} "
                 f"vol_mult={vol_mult} final_qty={qty}")
        if qty < 0.001:
            return
...
def main():
    ...
    post_feed("Engine v3.7 Online", f"Trading engine v3.7 started (OptimizedThresholds) (PortfolioCap+VolAdj+Benchmark). {len(STRATEGIES)} strategies.", "success")
